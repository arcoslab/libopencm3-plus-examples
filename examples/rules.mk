...
ifeq ($(strip $(OPENCM3_PLUS_DIR)),)
# user has not specified the library path, so we try to detect it

# where we search for the library
LIBPATHS := ./libopencm3-plus ../../../../libopencm3-plus ../../../../../libopencm3-plus

OPENCM3_PLUS_DIR := $(wildcard $(LIBPATHS:=/))
OPENCM3_PLUS_DIR := $(firstword $(OPENCM3_PLUS_DIR))

ifeq ($(strip $(OPENCM3_PLUS_DIR)),)
$(warning Cannot find libopencm3 library in the standard search paths.)
$(error Please specify it through OPENCM3_PLUS_DIR variable!)
endif
endif

ifeq ($(V),1)
$(info Using $(OPENCM3_PLUS_DIR) path to library)
endif
...
DEFS		+= -I$(OPENCM3_PLUS_DIR)/include
LDFLAGS		+= -L$(OPENCM3_PLUS_DIR)/lib
